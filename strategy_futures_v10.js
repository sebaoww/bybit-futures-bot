...
} = require('./indicators_v10'); // usa il file indicatori v10 personalizzato
...
function analyzeSignalV10(candles, volumes, lastTradeDirection = null, lastTradeTime = 0, currentTime = 0) {
    ...
    if (candles.length < 200 || close.length < 200) {
      logger.warn(`📉 Dati insufficienti (${candles.length} candele) — ne servono almeno 200`);
      return { signal: null, reentry: null, indicators: {} };
    }
    ...
    if (![ema9, ema25, ema200, rsi, macdHist].every(v => typeof v === 'number' && !isNaN(v))) {
      logger.warn('❌ Indicatori non validi o incompleti — salta analisi V10');
      return { signal: null, reentry: null, indicators: {} };
    }
    ...
    const longCond = ema9 > ema25 && rsi > 53 && adx > 18 && supertrend === true && macdHist > 0 && trendUp && volumeSpike;
    const shortCond = ema9 < ema25 && macdHist < 0 && supertrend === false && !trendUp && volumeSpike;
